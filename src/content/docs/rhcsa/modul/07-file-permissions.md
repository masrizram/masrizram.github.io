...
```bash
setfacl -m urwx file.txt     # beri andi akses rwx
setfacl -m grx file.txt       # beri group dev akses rx
getfacl file.txt                   # lihat ACL
setfacl -x u:andi file.txt         # hapus ACL user andi
setfacl -R -m dandi:rwx /data   # default ACL (warisan) untuk direktori
```
